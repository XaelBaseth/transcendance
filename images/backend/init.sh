...
if [ -z "$(python3 manage.py shell -c "from django.contrib.auth import get_user_model; \
	 print(get_user_model().objects.filter(username='$DJANGO_SUPERUSER_USERNAME').exists())")" ]; then
